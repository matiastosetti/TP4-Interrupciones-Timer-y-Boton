...
int main(void) { //sólo se inicializa hardware y no hace nada el bucle while y
	//todo lo que hace depende de la interrupción que llaman a funciones implementadas en el mainc Si esas funciones fueran de extenso código, comoanalizar la trama, en la interrupción cuento el tiempo en el que debería preguntar si debo analizar la trama y si debo recogerla para guardar el dato (el concepto es hacer lo minimo posible) y en el main.c analizo la trama.
	bsp_init();

	while (1) {
		bsp_delayMS(10); //demora el while 1 ms
		led_toggle(1); //led 1 titilando en el background
